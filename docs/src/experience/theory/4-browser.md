...
* **构建 DOM 树**
  ...
  * 浏览器将 HTML 文本流解析为 DOM 节点，并构建一棵 DOM 树。
  * 如果遇到 JavaScript（`<script>`）标签，可能会阻塞 DOM 树的构建，直到脚本执行完成（除非脚本有 async 或 defer 属性）

* **构建 CSSOM 树**

  * 浏览器解析所有样式表（包括外部 CSS 和内联样式），生成 CSSOM。
  * 如果 CSS 文件没有加载完成，可能会阻塞后续的渲染过程。

* **生成渲染树**

  * DOM 和 CSSOM 结合，生成一个渲染树（Render Tree）。
  * 渲染树的每个节点是一个可见元素，包含其视觉样式（颜色、尺寸等）。
  * 不可见的元素（如 display: none 的节点）不会加入渲染树。

* **布局/回流**

  * 确定每个元素的几何信息（位置和大小）。
  * 从根节点开始，为每个节点计算其在视口中的确切位置。
